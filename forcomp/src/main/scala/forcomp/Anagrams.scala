...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (char, count) :: tail => {
      ...
      tailCombinations ++ (for (remain <- tailCombinations; i <- 1 to count) yield (char, i) :: remain)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    val newMap = y.foldLeft(xMap) { (currentMap, pair) =>
      val (char, freq) = pair
      val updatedFreq = currentMap(char) - freq
      if (updatedFreq > 0) currentMap.updated(char, updatedFreq) else currentMap - char
    }

    newMap.toList.sortBy(_._1)
  }
    ...
    def helper(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        for {
          ...
          rest <- helper(subtract(occurrences, wordOccurrences(word)))
        } yield word :: rest
      }
    }
